...
namespace narvalengine {
	...
	Triangle::Triangle(float* index1, float* index2, float* index3, Material* material) {
		this->material = material;
		vertexData[0] = index1;
		vertexData[1] = index2;
		vertexData[2] = index3;
	...
	Triangle::Triangle(float* index1, float* index2, float* index3) {
		vertexData[0] = index1;
		vertexData[1] = index2;
		vertexData[2] = index3;
	}
	...
	glm::vec2 Triangle::samplePointOnTexture(glm::vec3 pointOnSurface) {
		if (vertexLayout == nullptr || !vertexLayout->contains(VertexAttrib::TexCoord0))
			...
			if (res.x < 0 || res.y < 0)
				LOG(ERROR) << "Invalid negative UV texture coordinates were generated by this Triangle. Values were:" << toString(res);
