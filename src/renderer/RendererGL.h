...
namespace narvalengine {
	static GLenum filterFlag(int flags, int mask, int shift) {
	...
	public:
		...
		void render(ProgramHandler program, RenderState *renderState) {
			...
			GLbitfield clear = GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT;
			glClear(clear);
			...
			while (!renderState->models.empty()) {
				...
				for (MeshHandler mesh : model.meshes) {
					...
					glDrawElements(GL_TRIANGLES, ib.size / 4, GL_UNSIGNED_INT, 0);
